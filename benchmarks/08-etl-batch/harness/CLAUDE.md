# CLAUDE.md — Data-Engineering ETL Batch Job · HARNESS

This is the **benchmark harness** for benchmark 08. Its job is to drive the
...
A deterministic multi-GB dataset generator (fixed seed, configurable row count + key cardinality); job-completion-time + GC-on-large-heap collector.

## Load profiles

- `small` — laptop-scale dataset, single-JVM.
- `large` — multi-GB, exercises large heaps + off-heap.
- `offheap` — Arrow-backed run for GC contrast.

Every generator takes a **fixed seed** — runs must be reproducible.

## Mode-specific KPIs (added to the core KPI block)

- Total job-completion time and records/sec.
- Peak memory: heap and native, including off-heap caches/buffers.
- GC pause times, frequency, total GC time on large heaps.
...
> Current scope: GC matrix on HotSpot 21 is the focus. GraalVM JIT comparison deferred to the harness registry. Shares dataset/harness components with 03-streaming-analytics where possible.
