# CLAUDE.md — Massive-Concurrency Chat / Collaboration Server (Loom) · HARNESS
...
This is the **benchmark harness** for benchmark 06. Its job is to drive the
...
WebSocket connection ramp generator (toward millions of sessions) + broadcast-latency measurement under spike loads (mass join/leave).

## Load profiles

- `ramp-connections` — climb concurrent sessions to find the ceiling.
- `broadcast-spike` — mass join/leave + fan-out bursts measuring p99–p99.99 delivery.

Every generator takes a **fixed seed** — runs must be reproducible.

## Mode-specific KPIs (added to the core KPI block)

- Max sustainable concurrent sessions and messages/sec.
- Broadcast-delivery latency distribution (p99–p99.99) under spikes.
- Memory per connection and GC behaviour as concurrency scales.
...
> Current scope: Loom vs NIO is the live axis (Loom GA in 21, no preview flags). GraalVM comparison deferred to the harness registry.
