# CLAUDE.md — High-Throughput Fintech Ledger Service · HARNESS
...
This is the **benchmark harness** for benchmark 01. Its job is to drive the
...
Gatling (Java DSL) for the HTTP/gRPC mix; a deterministic transaction-stream generator with salary-day bursts, campaign spikes, and read-heavy periods (fixed seed).

## Load profiles

- `steady` — flat mixed read/write load.
- `salary-day-burst` — short heavy write bursts over a read-heavy baseline.
- `campaign-spike` — correlated spikes across accounts.

Every generator takes a **fixed seed** — runs must be reproducible.

## Mode-specific KPIs (added to the core KPI block)

- End-to-end latency broken down app vs DB vs fraud-API.
- Warmup curve: time until throughput + latency stabilise per GC.
...
> Current scope: GraalVM JIT / native-image comparison is deferred; only the GC matrix (G1/ZGC/Shenandoah on HotSpot 21) is active now.
