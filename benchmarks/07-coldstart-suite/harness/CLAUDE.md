# CLAUDE.md — Serverless / CRaC / Native-Image Cold-Start Suite · HARNESS
...
This is the **benchmark harness** for benchmark 07. Its job is to drive the
...
A cold-start harness that spawns a fresh process/container per invocation and times process-start→first-successful-request, plus a warm-invocation loop; collects RSS and (later) checkpoint size / binary size.

## Load profiles

- `cold` — N repeated fresh-process cold starts (active now, HotSpot).
- `warm` — repeated invocations reusing one process.
- `cold-crac` / `cold-native` — defined, run in the deferred phase.

Every generator takes a **fixed seed** — runs must be reproducible.

## Mode-specific KPIs (added to the core KPI block)

- Cold-start latency (process start → first successful request).
- Warm-invocation latency distribution.
- Footprint: RSS per mode; (deferred) checkpoint-file size for CRaC, binary size for native.
- (Deferred) checkpoint create time, restore time + success rate, open-resource failure modes.
...
> Current scope: CRaC + native-image are the reason this benchmark exists but are explicitly OUT OF SCOPE now. Deliver the functions, the HotSpot cold/warm harness, and the `LaunchMode` seam. Do NOT add `org.crac.*` or native-image config to app/ yet.
