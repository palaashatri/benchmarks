# AGENTS.md — JVM Benchmark Suite (suite root)
...
- **Cohort B — benchmarks 09–13:** Maven, JDK 17 (09–11, 13) or JDK 25 (12),
  ...
  superseded by each benchmark's own AGENTS.md.

> **Read this file first, then the `AGENTS.md` inside the specific benchmark
...
```
jvm-bench-suite/
  AGENTS.md                      <- you are here (suite-wide)
  benchmarks/
    # Cohort A — Gradle / JDK 21
    01-fintech-ledger/
      AGENTS.md                  <- benchmark root: scenario, contract, how the pieces fit
      app/      AGENTS.md         <- the shippable application (no harness deps)
      harness/  AGENTS.md         <- load gen, metrics capture, KPI extraction, runtimes
...
> AGENTS.md files are authoritative for build commands and toolchain config.
...
- `app/` builds, tests, and runs on OpenJDK 21 with no harness on the classpath,
  and can be zipped out and run elsewhere using only its own `AGENTS.md`.
